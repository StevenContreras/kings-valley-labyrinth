...
describe("isValidMove", () => {
    it("returns false if a piece with the given ID does not exist", () => {
        ...
        const result = isValidMove(startingGameState, nonExistentPieceId, destination);
        expect(result).toBe(false);
    });
    it("returns true if a piece with a valid ID exists, belongs to the current player, and has a valid destination", () => {
        const destination: Position = { row: 3, col: 4 };
        const pieceId = "p1-guard-2";
        const result = isValidMove(startingGameState, pieceId, destination);
    ...
    it("returns false when a valid current-player piece is blocked to the west", () => {
    ...
    it("returns false when a valid current-player piece is a guard and the destination is the valley", () => {
        ...
        const result = isValidMove(randomGameState, pieceId, destination);
        expect(result).toBe(false);
    });
    it("returns true when a valid current-player piece is a king and the destination is the valley", () => {
        const destination: Position = { row: 3, col: 3 };
...
describe("isWinningMove", () => {
    it("returns false if the move is not a winning move", () => {
        ...
        const result = isWinningMove(piece!, destination);
        expect(isValidMove(startingGameState, pieceId, destination)).toBe(true);
        expect(result).toBe(false);
    });
    it("returns true if the move is a winning move", () => {
        ...
        expect(piece?.owner).toBe(randomGameState.currentPlayer);
        expect(piece?.role).toBe("KING");
        expect(isValidMove(randomGameState, pieceId, destination)).toBe(true);

        expect(isWinningMove(piece!, destination)).toBe(true);
    });
});
